...
<div class="nav-bar">
	...
	<div class="container">
		...
		<div class="right-content columns eleven">
			...
			<div class="katagoriwarnasatu" id="title"><h1><?php echo $row['nama_kategori']; ?></h1></div>
            <a href="isi_post_member.php?id=<?php echo $id; ?>" id="add_thread">
                    <button type="submit">Add thread</button>
                </a>
                    <?php
                        $id = $_GET['id'];
                        $query = "SELECT * FROM kategori k JOIN post a ON k.id_kategori = a.id_kategori WHERE k.id_kategori = $id";
                        $result = mysql_query($query);
                        while ($row = mysql_fetch_assoc($result)){
                    ?>
					...
					<a href="detail.php?id=<?php echo $row['id_post']; ?>"><li><h2><?php echo $row['judul']; ?></h2></li></a>
