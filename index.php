...
<body>
	...
	<div class="container">
		<div class="left-content">
			<h2 class="katagoriwarna">
				Kategori
                    ...
                    ?>
				    <ul>
					<a class="kategorisatu" href="konten.php?id=<?php echo $row['id_kategori']?>"><li><?php echo $row['nama_kategori']?></li></a>
					</ul>
                    <?php
                        }
                    ?>
		...
		<div class="right-content columns eleven">
			<div class="katagoriwarnasatu" id="title"><h1>New Thread</h1></div>
					<?php
                        // $id = $_GET['id'];
                        $query = "SELECT * FROM post WHERE status='ENABLE' ORDER BY id_post DESC LIMIT 10";
                        $result = mysql_query($query);
                        while ($row = mysql_fetch_assoc($result)){
                    ?>
                    <ul>
